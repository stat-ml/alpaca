...
class SuperconductData:
    """Superconductivity, UCI dataset"""

    def __init__(self, use_cache=False, val_split=0.2):
        self.use_cache = use_cache
        cache_dir = path.join(DATA_DIR, "dataloader/data/superconduct")
    ...
    def _build_dataset(self, cache_dir):
        data_path = download(cache_dir, "superconduct.zip", URL)
        with zipfile.ZipFile(data_path, "r") as zip_ref:
            zip_ref.extractall(cache_dir)
        file_path = path.join(cache_dir, "train.csv")
