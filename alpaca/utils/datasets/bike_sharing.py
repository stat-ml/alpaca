...
class BikeSharingHourData:
    """Bike Sharing, UCI dataset"""

    def __init__(self, use_cache=False, val_split=0.2):
        self.use_cache = use_cache
        cache_dir = path.join(DATA_DIR, "dataloader/data/bike_sharing")
    ...
    def _build_dataset(self, cache_dir):
        data_path = download(cache_dir, "bike_sharing.zip", URL)
        with zipfile.ZipFile(data_path, "r") as zip_ref:
            zip_ref.extractall(cache_dir)
        file_path = path.join(cache_dir, "hour.csv")
        self.df = pd.read_csv(file_path)
        self.df.drop(['temp', 'dteday', 'registered', 'casual'],
                     axis = 1, 
                     inplace = True)
