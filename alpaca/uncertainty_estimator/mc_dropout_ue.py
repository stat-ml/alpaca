...
class MCDUE:
    def __init__(self, model, nn_runs=100, acquisition='std', output_size=1, dropout_rate=.5, dropout_mask=None):
        """
        :param model: neural network to work with
        :param nn_runs:  number of forward passes for uncertainty estimation
        :param dropout_rate:  dropout rate during uncertainty estimation
        :param acquistion: should be 'std' for regression,
            and one of the 'var_ratio', 'var_soft' or 'bald' for classification
        :param dropout_mask: if had custom s
        """
        # for classification and acquistion take a look at the bald.py

    def estimate(self, x_pool):
        """
        :param x_pool: tensor or loader with points to estimate
        :return: predictions (NxC), uncertainty estimation (N),
            where N - number of points in X_pool
            C - output_size (number of classes for classifications or outputs for regresssion)
        """

    def reset(self):
        """
        resets the mask if they has some state (dpp-related)
        :return: None
        """
